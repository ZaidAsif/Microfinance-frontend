...
const LoanProceedModal: React.FC<LoanProceedModalProps> = ({
  ...
  const validateCNIC = (cnic: string) => /^[0-9]{13}$/.test(cnic); // Must be 13 digits
  const validateEmail = (email: string) => /\S+@\S+\.\S+/.test(email); // Simple email validation
  ...
  const handleSubmit = async () => {
    ...
    const { data } = await axios.post(`${BASIC_URL}loan/apply`, { email, name, cnic, category: loanCategory, subcategory: loanSubcategory, amount: loanAmount, period: loanPeriod, monthlyInstallment })
    console.log(data);
