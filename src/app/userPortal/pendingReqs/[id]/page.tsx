...
export default function LoanDetails({ params }: { params: { id: string } }) {
    ...
    const validate = () => {
        ...
        if (!/^\d{13}$/.test(guarantor1Cnic)) newErrors.guarantor1Cnic = "CNIC must be 13 digits";
        if (!/^\d{13}$/.test(guarantor2Cnic)) newErrors.guarantor2Cnic = "CNIC must be 13 digits";
        if (!/^\d{11,13}$/.test(phone)) newErrors.phone = "Phone must be 11-13 digits";
        setErrors(newErrors);
        return Object.keys(newErrors).length === 0;
    };

    // Separate PDF generation logic
    const generatePdfSlip = async (userId: string, loanId: string, token: string) => {
        setPdfLoading(true);
        try {
            const { data } = await axios.post(
            ...
            console.log(data)
            // setPdfUrl(data?.data?.slipUrl ? `${BASIC_URL.replace(/\/api\/?$/, "")}${data.data.slipUrl}` : null);
            // setFormSuccess("Appointment slip generated! You can download it below.");
        } catch (error: any) {
            setFormError(
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // 2. Trigger PDF generation separately
            generatePdfSlip(user._id, id, token);
        } catch (error: any) {
            setFormError(
                error?.response?.data?.message ||
                error?.response?.data?.msg ||
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 flex flex-col md:flex-row">
            ...
            <main className="flex-1 p-4 sm:p-8 py-20 max-w-2xl mx-auto">
                <h1 className="text-3xl font-bold text-green-700 mb-6 text-center">Loan Request Details</h1>
                {!loanDetails ? (
                    ...
                    <div className="bg-white rounded-2xl shadow-xl p-8 mb-10 border border-gray-100 transition hover:shadow-2xl">
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                            <div>
                                <h2 className="text-xl font-semibold text-blue-800 mb-1">{loanDetails.category} <span className="text-gray-500">({loanDetails.subcategory})</span></h2>
                                <span className="inline-block px-3 py-1 rounded-full bg-yellow-100 text-yellow-700 text-xs font-bold">{loanDetails.status}</span>
                            </div>
                            <div className="flex flex-col gap-1 text-right">
                                <span className="text-lg font-bold text-green-600">PKR {loanDetails.amount}</span>
                                <span className="text-gray-600 text-sm">Period: {loanDetails.period} years</span>
                                <span className="text-gray-600 text-sm">Installment: PKR {loanDetails.monthlyInstallment || "N/A"}</span>
                            </div>
                        </div>
                        <div className="text-gray-700 text-sm">
                            <div><strong>Applied On:</strong> {loanDetails.createdAt ? new Date(loanDetails.createdAt).toLocaleDateString() : "N/A"}</div>
                            <div><strong>Purpose:</strong> {loanDetails.purpose || "N/A"}</div>
                        </div>
                    </div>
                )}
                ...
                <form
                    ...
                    autoComplete="off"
                >
                    <h2 className="text-xl font-semibold text-blue-800 mb-4">Guarantors' Information</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
                        <div className="bg-gray-50 p-4 rounded-lg border hover:shadow-md transition">
                            ...
                            />
                            <input
                                className={`w-full mb-2 px-3 py-2 border rounded focus:ring-2 focus:ring-green-200 transition ${errors.guarantor1Cnic ? "border-red-500" : ""}`}
                                placeholder="CNIC"
                                value={guarantor1Cnic}
                                onChange={e => setGuarantor1Cnic(e.target.value.replace(/\D/g, '').slice(0, 13))}
                                maxLength={13}
                                required
                            />
                            {errors.guarantor1Cnic && <p className="text-red-500 text-xs mb-2">{errors.guarantor1Cnic}</p>}
                        </div>
                        <div className="bg-gray-50 p-4 rounded-lg border hover:shadow-md transition">
                            ...
                            />
                            <input
                                className={`w-full mb-2 px-3 py-2 border rounded focus:ring-2 focus:ring-green-200 transition ${errors.guarantor2Cnic ? "border-red-500" : ""}`}
                                placeholder="CNIC"
                                value={guarantor2Cnic}
                                onChange={e => setGuarantor2Cnic(e.target.value.replace(/\D/g, '').slice(0, 13))}
                                maxLength={13}
                                required
                            />
                            {errors.guarantor2Cnic && <p className="text-red-500 text-xs mb-2">{errors.guarantor2Cnic}</p>}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
                        ...
                        />
                        <input
                            className={`w-full px-3 py-2 border rounded focus:ring-2 focus:ring-green-200 transition ${errors.phone ? "border-red-500" : ""}`}
